  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
        <button onClick={() => setModal("imagem")} style={{ padding: "9px 16px", background: "#7c3aed", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>+ Nova tentativa</button>
        <button onClick={() => setModal("fluxo")} style={{ padding: "9px 16px", background: "transparent", color: "#7c3aed", border: "1px solid rgba(124,58,237,0.4)", borderRadius: 8, fontSize: 14, cursor: "pointer" }}>+ Novo fluxo</button>
      </div>

      {fluxos.length > 0 && (
        <section style={{ marginBottom: 32 }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#e2e8f0", marginBottom: 12 }}>Fluxos documentados</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {fluxos.map((f) => (
              <div key={f.id} style={{ background: "#111118", border: "1px solid #1e1e2e", borderRadius: 10, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <p style={{ color: "#e2e8f0", fontWeight: 600 }}>{f.nome}</p>
                  <span style={{ color: "#64748b", fontSize: 12 }}>{f.ferramentaRef?.nome ?? f.ferramenta} · confiança {f.confianca}/5</span>
                </div>
                <p style={{ color: "#94a3b8", fontSize: 13 }}>{f.objetivo}</p>
                <p style={{ color: "#64748b", fontSize: 12, marginTop: 8, lineHeight: 1.5 }}>{f.instrucoes.slice(0, 160)}{f.instrucoes.length > 160 ? "…" : ""}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12 }}>
        {initial.map((img) => (
          <div key={img.id} style={{ background: "#111118", border: "1px solid #1e1e2e", borderRadius: 12, padding: 20 }}>
            ...
            <span style={{ display: "inline-block", marginTop: 8, padding: "2px 8px", background: img.status === "aprovada" ? "#34d39920" : "#7d899c20", color: img.status === "aprovada" ? "#34d399" : "#7d899c", borderRadius: 4, fontSize: 11 }}>{img.status}</span>
          </div>
        ))}
        {initial.length === 0 && (
      ...
      {modal === "imagem" && (
        <div onClick={() => !saving && setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "60px 20px", zIndex: 50 }}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={saveImagem} style={{ width: "100%", maxWidth: 480, background: "#111118", border: "1px solid #1e1e2e", borderRadius: 12, padding: 24 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#e2e8f0", marginBottom: 18 }}>Nova tentativa</h2>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
              ...
              <button type="submit" disabled={saving} style={{ padding: "9px 16px", background: "#7c3aed", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer" }}>Salvar</button>
            </div>
          </form>
        </div>
      )}
      ...
      {modal === "fluxo" && (
        <div onClick={() => !saving && setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "60px 20px", zIndex: 50 }}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={saveFluxo} style={{ width: "100%", maxWidth: 480, background: "#111118", border: "1px solid #1e1e2e", borderRadius: 12, padding: 24 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#e2e8f0", marginBottom: 18 }}>Novo fluxo de imagem</h2>
            <div style={{ display: "grid", gap: 12 }}>
